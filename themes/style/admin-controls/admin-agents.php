				...
				foreach($DBcomp_agentEmails as $DBcomp_agentEmail){ 
                    ...
                    <tr>
                        <td class="tick_gen_cols">
							<?php echo $user_id_emailFound[0]->display_name; ?><br /><br />
                            <p>
                                 <a href="?action=agent_tickets&id=<?php echo $user_id_emailFound[0]->ID; ?>&pagenumber=1" class="button button-default btn btn-primary btn-sm"><?php _e('Agent Assigned Tickets(s)','mhelpdesk');?></a> 
                                 
                                 
                                 <a href="?action=delete&id=<?php echo $user_id_emailFound[0]->ID; ?>" class="btn btn-danger btn-sm"  
                                  		onclick="if(confirm('Are you sure, to delete this record?')) return true; else return false">Delete</a>
                                 
                                 
                                 <!--a href="?action=delete&id=<?php echo $user_id_emailFound[0]->ID; ?>"><?php _e('Delete','mhelpdesk');?></a--> 
                            </p>
                        </td>
                        <td class="tick_gen_cols"><?php echo $user_id_emailFound[0]->user_email; ?></td>
                    </tr>
<?php
            ...
            <td>
        		...
        		<?php foreach($unregEmailsArr as $unregisteredEmail){ ?>
                	<tr>
                        ...
                        <td class="tick_gen_cols">
	<?php echo $unregisteredEmail; ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  
    <a href="?action=delete&agent_email=<?php echo $unregisteredEmail; ?>" class="btn btn-danger btn-sm" 
                                  		onclick="if(confirm('Are you sure, to delete this record?')) return true; else return false">Delete</a> 
                        </td>
                    </tr>
                <?php } //foreach($unregEmailsArr as $unregisteredEmail) ?>
			...
			<table class="ticketswrap">
				<tr>
                    ...
                    <div class="viewticket_td">
						...
						(<?php $userAgent_DB_Obj = $wpdb->get_results("SELECT display_name FROM $wpdb->users WHERE ID = '$agent_id'"); echo $userAgent_DB_Obj[0]->display_name; ?>) 
