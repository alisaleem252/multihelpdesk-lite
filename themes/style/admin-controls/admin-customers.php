			...
			<table>
                ...
                <tr>
                	<td class="tick_gen_cols">
						<?php echo $customer_displaynameObj[0]->display_name; ?>
                        <br /><br />
                                <p>
                                 <a href="?action=customer_tickets&id=<?php echo $customerID; ?>&pagenumber=1" class="btn btn-primary btn-sm"><?php _e('View Customer Tickets','mhelpdesk');?></a>&nbsp;&nbsp;&nbsp;
                                 <a href="?action=edit&id=<?php echo $customerID; ?>" class="btn btn-warning btn-sm"><?php _e('Edit','mhelpdesk');?></a>&nbsp;&nbsp;&nbsp;
                                 <a class="red" href="?action=delete&id=<?php echo $customerID; ?>" class="btn btn-danger btn-sm" onclick="if(confirm('Are you sure, to delete this record?')) return true; else return false">
...
if($pages > 1){?>
                ...
                Pages: <a>&laquo;</a></li><?php
                    for($index = 1; $index <= $pages; $index++){?>
                            ...
                            <a href="?action=<?php echo $url_action_value.'&id='.$customer_id_GET.'&pagenumber='.$index; ?>" ><?php echo $index; ?></a>
    <?php			} // for($index = 1; $index <= $pages; $index++) ?>
        	...
        	<tr><td colspan="4" class="tick_gen_cols"><h2><?php echo $customer_displaynameObj[0]->display_name; ?></h2></td></tr>
            <tr>
 ...
 if ( $rating ){
    ...
    <?php
                ...
                <td class="tick_gen_cols"><?php echo ( isset($agent_displaynameObj[0]) ? $agent_displaynameObj[0]->display_name : __('No Assigned Yet','mhelpdesk')); ?></td>
                <td class="tick_gen_cols">
...
<?php	
		...
		else echo '<h1 ><br /><br />'.__('This Customer ','mhelpdesk').'('.$customer_displaynameObj[0]->display_name.') '.__('is blocked.','mhelpdesk').'</h1>';
...
<a href="<?php echo $_SERVER['HTTP_REFERER'];?>" ><?php _e("Go Back to Customer's Home Link",'mhelpdesk');?></a>
        <table>
            ...
            <tr>
                <td class="tick_gen_cols"><?php echo $customer_displaynameObj[0]->display_name; ?></td>
                <td class="tick_gen_cols">
...
$relevantCID_foundIndex = array_search($customer_id_GET, isset($compCustomerOpened_DB_arr[0]) ? $compCustomerOpened_DB_arr : array());
	...
	if($relevantCID_foundIndex > -1 ){
		...
		echo __('Are you sure to delete this ','mhelpdesk').'"'.$customer_displaynameObj[0]->display_name.'" '.__('Customer?','mhelpdesk').'<br />';
